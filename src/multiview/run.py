...
parser.add_argument('--lambda1', type = float, default = 0.1, help = 'trade-off hyperparameter of consistency loss')
parser.add_argument('--lambda2', type = float, default = 0.1, help = 'trade-off hyperparameter of cross entropy loss')
...
def main():
    ...
    for data_seed in range(1, args.test_time + 1):
        ...
        # Load view1, view2, and view3 with tensor
        X1, X2 ,X3 = torch.from_numpy(X_list[0]).float().cuda(), torch.from_numpy(X_list[1]).float().cuda(), torch.from_numpy(X_list[2]).float().cuda()
        
        # Build the model
        model = mvc(args, config)
        model.model_init()
        optimizer = torch.optim.Adam(
            itertools.chain(model.autoencoder1.parameters(), model.autoencoder2.parameters(), model.autoencoder3.parameters()),
        ...
        # Training
        acc, nmi, ari = model.train(config, logger, X1, X2,X3, gt, optimizer, device)
